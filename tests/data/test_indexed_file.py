...
def test_internal_get_signed_url_no_protocol_index_error(
    ...
    without a protocol gives IndexError
    """
    indexd_record_with_non_public_authz_and_public_acl_populated = {
        "urls": [f"{supported_protocol}://some/location"],
...
@pytest.mark.parametrize(
    "public_bucket_delete_indexd_client",
    ["gs", "s3", "gs_acl", "s3_acl", "https_azure"],
    indirect=True,
)
def test_delete_files_unable_to_get_file_name(app, public_bucket_delete_indexd_client):
    """
...
def test_delete_files_successful(app, public_bucket_delete_indexd_client):
    """
...
# supported_protocol, indexd_client_accepting_record
@pytest.mark.parametrize(
